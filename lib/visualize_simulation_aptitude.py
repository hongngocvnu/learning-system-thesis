...
def visualize_results():
    ...
    with open('simulation_results_ts_aptitude.json', 'r') as f:
        results = json.load(f)

    # Extract data
    aptitude_levels = ['weak', 'medium', 'strong']
    policies = ['Thompson', 'HDoC', 'Random']
    ...
    # Extract all metrics
    for aptitude in aptitude_levels:
        for policy in policies:
            accuracy_data[policy].append(results[aptitude][policy]['accuracy'])
            avg_questions_data[policy].append(results[aptitude][policy]['avg_questions'])
            f1_score_data[policy].append(results[aptitude][policy]['f1_score'])
            precision_data[policy].append(results[aptitude][policy]['precision'])
    ...
    x = np.arange(len(aptitude_levels))
    bar_width = 0.25
    ...
    ax2.set_xlabel('Aptitude Level')
    ax2.set_ylabel('Accuracy')
    ax2.set_title('Accuracy')
    ax2.set_xticks(x)
    ax2.set_xticklabels(aptitude_levels)
    ax2.set_ylim(0, 1.05)
    ...
    ax3.set_xlabel('Aptitude Level')
    ax3.set_ylabel('F1 Score')
    ax3.set_title('F1 Score')
    ax3.set_xticks(x)
    ax3.set_xticklabels(aptitude_levels)
    ax3.set_ylim(0, 1.05)
    ...
    ax4.set_xlabel('Aptitude Level')
    ax4.set_ylabel('Precision')
    ax4.set_title('Precision')
    ax4.set_xticks(x)
    ax4.set_xticklabels(aptitude_levels)
    ax4.set_ylim(0, 1.05)
    ...
    fig.tight_layout()
    plt.savefig('simulation_results_aptitude.png', dpi=300, bbox_inches='tight')
    plt.close()

    print("\nCombined visualization saved as 'simulation_results_aptitude.png'")
