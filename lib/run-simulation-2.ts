...
interface PolicyResultsByThreshold {
  [threshold: string]: {
...
async function runPolicySimulation(
  ...
  for (let i = 0; i < numRuns; i++) {
    ...
    const { mastery: masteryGroundTruth } = await generateStudentMasteryAndParams(
      graph, 0.8, 0.2, config.masteryThreshold
    );
...
async function main() {
  const baseConfig: AssessmentConfig = {
    ...
    preSampleCount: 1,
    maxQuestions: 80,
    sampling_policy: 'Thompson' as 'Thompson' | 'HDoC' | 'Random' // This will be set per policy
  };
  ...
  const masteryThresholds: number[] = [];
  for (let i = 2; i <= 9; i++) {
    masteryThresholds.push(i / 10);
  }

  const results: PolicyResultsByThreshold = {};
