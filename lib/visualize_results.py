...
def visualize_results():
    ...
    with open('simulation_results_ts.json', 'r') as f:
        results = json.load(f)

    # Extract data
    kc_counts = list(range(2, 17))  # 2 to 16 KCs
    ...
    for policy in policies:
        accuracy_data[policy] = results[policy]['accuracy']
        avg_questions_data[policy] = results[policy]['avg_questions']
        f1_score_data[policy] = results[policy]['f1_score']
        precision_data[policy] = results[policy]['precision']
    ...
    ax1.set_xlabel('Number of Knowledge Components')
    ax1.set_ylabel('Question Count')
    ax1.set_title('Average Questions Asked')
    ax1.set_xticks(x)
    ax1.set_xticklabels(kc_counts)
    ax1.set_ylim(0, 80)
    ...
    ax2.set_xlabel('Number of Knowledge Components')
    ax2.set_ylabel('Accuracy')
    ax2.set_title('Accuracy')
    ax2.set_xticks(x)
    ax2.set_xticklabels(kc_counts)
    ax2.set_ylim(0, 1.05)
    ...
    ax3.set_xlabel('Number of Knowledge Components')
    ax3.set_ylabel('F1 Score')
    ax3.set_title('F1 Score')
    ax3.set_xticks(x)
    ax3.set_xticklabels(kc_counts)
    ax3.set_ylim(0, 1.05)
    ...
    ax4.set_xlabel('Number of Knowledge Components')
    ax4.set_ylabel('Precision')
    ax4.set_title('Precision')
    ax4.set_xticks(x)
    ax4.set_xticklabels(kc_counts)
    ax4.set_ylim(0, 1.05)
    ...
    fig.tight_layout()
    plt.savefig('simulation-ts-kc-2.png', dpi=300, bbox_inches='tight')
    plt.close()

    print("\nCombined visualization saved as 'simulation-ts-kc-2.png'")
