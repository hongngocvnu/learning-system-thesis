...
def main():
    # Vary masteryThreshold from 0.2 to 0.9
    mastery_thresholds = np.arange(0.2, 0.91, 0.1)
    kc_count = 16
    delta = 0.15
    pre_sample_count = 1
    max_questions = 80
    ...
    for mastery in mastery_thresholds:
        print(f"\nSimulating with masteryThreshold={mastery:.2f}, KC count={kc_count}, delta={delta}")
        config = AssessmentConfig(
            masteryThreshold=mastery,
            confidenceThreshold=delta,
            preSampleCount=pre_sample_count,
            maxQuestions=max_questions,
            sampling_policy='any'
        )
        for policy_class in [ThompsonSamplingPolicy, HDoCPolicy, RandomPolicy]:
            print(f"Running {policy_class().name}...")
            policy_results = run_policy_simulation(policy_class, num_kcs=kc_count, num_runs=num_runs, config=config)
            results[policy_class().name]['accuracy'].append(policy_results[0])
            results[policy_class().name]['avg_questions'].append(policy_results[1])
            results[policy_class().name]['std_questions'].append(0)
            print(f"{policy_class().name} Results:")
            print(f"Accuracy: {policy_results[0]:.3f}")
            print(f"Avg Questions: {policy_results[1]:.1f}")
            print("---")

    # Plot results
    plt.figure(figsize=(12, 4))  # Tăng kích thước để có chỗ cho legend
    x = np.arange(len(mastery_thresholds))
    bar_width = 0.25
    ...
    ax1.set_xticks(x)
    ax1.set_xticklabels([f"{v:.1f}" for v in mastery_thresholds])  # Hiển thị 1 số thập phân
    ...
    ax2.set_yticks(np.arange(0, 1.1, 0.1))  # Đặt bước nhảy 0.1 cho trục y

    plt.title('Question Count and Accuracy vs Mastery Threshold', fontsize=14, fontweight='bold', y=1.05)

    # Tạo figure riêng cho legend
    ...
    plt.tight_layout()
    plt.savefig('simulation_results-2.png', dpi=200, bbox_inches='tight')
    plt.close()

    # Lưu legend riêng
    legend_fig.savefig('simulation_legend-2.png', dpi=200, bbox_inches='tight')
    plt.close(legend_fig)

    # Save detailed results
    with open('simulation_results-2.json', 'w') as f:
