import { HDoCPolicy, ThompsonSamplingPolicy, RandomPolicy } from './adaptive-assessment.js';
import { createRandomDag, generateStudentMasteryAndParams, simulateAnswer } from './simulation-helpers.js';
...
// --- Main Function for Threshold Simulation ---
async function main() {
    const baseConfig = {
        masteryThreshold: 0.7, // This will be overridden in the loop
        confidenceThreshold: 0.15,
        preSampleCount: 1,
        maxQuestions: 80,
        sampling_policy: 'Thompson' // This will be set per policy
    };
    const numKcs = 16; // Fixed number of KCs
    const numRuns = 1000; // Number of runs per threshold and policy
    // Mastery thresholds to test
    const masteryThresholds = [];
    for (let i = 2; i <= 9; i++) {
        masteryThresholds.push(i / 10);
    }
    const results = {};
    ...
    for (const threshold of masteryThresholds) {
        ...
        console.log(`\nSimulating with Mastery Threshold: ${thresholdKey}...`);
        for (const policyInfo of policiesToTest) {
            console.log(`Running ${policyInfo.name}...`);
            const currentConfig = {
                ...baseConfig,
                masteryThreshold: threshold,
                sampling_policy: policyInfo.name
            };
            const simResults = await runPolicySimulation(policyInfo.class, numKcs, numRuns, currentConfig);
            results[thresholdKey][policyInfo.name] = simResults;
            console.log(`${policyInfo.name} Results for threshold ${thresholdKey}:`);
    ...
    const resultsFileName = 'simulation_results_ts_threshold.json';
    fs.writeFileSync(resultsFileName, JSON.stringify(results, null, 2));
    console.log(`\nSimulation results saved to ${resultsFileName}`);
    // Note: Visualization script needs to be updated to handle this new data structure.
    // For now, we just generate the JSON.
}
// Run the simulation
