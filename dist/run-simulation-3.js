...
// --- Helper Functions ---
function getRandomAptitude(aptitudeLevel) {
...
async function runPolicySimulation(PolicyClass, numKcs, numRuns, config, aptitudeLevel) {
    ...
    for (let i = 0; i < numRuns; i++) {
        ...
        const { mastery: masteryGroundTruth } = await generateStudentMasteryAndParams(graph, aptitude, 0.2, config.masteryThreshold);
...
async function main() {
    ...
    const numKcs = 16;
    const numRuns = 1000;
    const aptitudeLevels = ['weak', 'medium', 'strong'];
    const results = {};
    ...
    for (const aptitude of aptitudeLevels) {
        ...
        for (const policyInfo of policiesToTest) {
            ...
            const simResults = await runPolicySimulation(policyInfo.class, numKcs, numRuns, currentConfig, aptitude);
