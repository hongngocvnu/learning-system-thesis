...
# # Chọn LO theo policy
# def select_lo(policy, mastery_probs):
#     if policy == 'Random':
#         return random.randint(0, len(mastery_probs)-1)
#     elif policy == 'Thompson':
#         # Chọn LO có xác suất thấp nhất (thompson sampling đơn giản)
#         return np.argmin(mastery_probs)
#     elif policy == 'HDoC':
#         # Chọn LO có xác suất gần ngưỡng mastery nhất nhưng chưa đạt
...
# # Mô phỏng 1 học viên
# def simulate_student(lo_count, policy, ground_truth_lo):
#     # Khởi tạo Beta cho mỗi LO
#     a = np.ones(lo_count)
#     b = np.ones(lo_count)
#     total_questions = 0
...
#             # Nếu đã xác định đúng LO yếu ground truth, dừng
...
#             # Chọn LO để hỏi
#             lo = select_lo(policy, mastery_probs)
#             # Sinh kết quả trả lời: nếu là LO yếu ground truth thì xác suất đúng thấp
#             if lo == ground_truth_lo:
#                 correct = np.random.rand() < 0.3  # 30% đúng nếu là LO yếu
#             else:
#                 correct = np.random.rand() < 0.8  # 80% đúng nếu không yếu
#             # Cập nhật Beta
#             if correct:
#                 a[lo] += 1
#             else:
#                 b[lo] += 1
#             total_questions += 1
#     # Nếu hết câu hỏi mà chưa xác định được LO yếu
#     mastery_probs = a / (a + b)
#     predicted_weak_lo = np.argmin(mastery_probs)
...
#                 num_questions, predicted_weak_lo = simulate_student(lo_count, policy, ground_truth_lo)
...
#     output_file = os.path.join(output_dir, f'beta-simulation-summary-{timestamp}.json')
