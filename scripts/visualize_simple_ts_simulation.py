...
def find_latest_result():
    ...
    files = glob.glob(os.path.join(results_dir, 'simple-ts-simulation-KC-sweep-*.json'))
    if not files:
        raise FileNotFoundError('Không tìm thấy file kết quả simple-ts-simulation-KC-sweep!')
...
def main():
    ...
    with open(result_file, 'r') as f:
        data = json.load(f)
    kc_counts = sorted(list(set(d['kcCount'] for d in data)))
    avg_questions = {policy: [] for policy in policies}
    accuracies = {policy: [] for policy in policies}
    for kc in kc_counts:
        for i, policy in enumerate(policies):
            entry = next(d for d in data if d['kcCount'] == kc and d['policy'] == policy)
    ...
    plt.title('KC count vs. Question Count and Accuracy')
    plt.tight_layout()
    plt.savefig('simple_ts_simulation.png')
    plt.show()
