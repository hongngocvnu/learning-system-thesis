...
class HDoCPolicy(SamplingPolicy):
    def __init__(self, K_total_kcs_in_unit: int, delta: float, mastery_threshold: float):
        super().__init__("HDoC")
        self.K_total_kcs = K_total_kcs_in_unit
        self.delta = delta
        self.mastery_threshold = mastery_threshold
        self.adaptive_round_count = 0
        self.debug = False  # Bật nếu muốn log

    def reset_adaptive_round_count(self):
        self.adaptive_round_count = 0
    ...
    def selectTopic(self, topics: List[Topic], studentState: StudentState) -> int:
        ...
        if not available_topic_objects:
            return -1
        self.adaptive_round_count += 1
        t_adaptive = max(2, self.adaptive_round_count)
        min_ucb_score = float('inf')
        selected_topic_id = -1
        for topic_obj in available_topic_objects:
            ...
            exploration_term = np.sqrt((2 * np.log(t_adaptive)) / N_i_t)
            ucb_score = mu_hat + exploration_term
            if ucb_score < min_ucb_score:
                min_ucb_score = ucb_score
                selected_topic_id = topic_obj.id
        if selected_topic_id == -1 and available_topic_objects:
            selected_topic_id = random.choice([t.id for t in available_topic_objects])
        return selected_topic_id

    def updateAfterAnswer(self, topicId, alpha, beta, masteryThreshold, delta, K_total_kcs_in_unit, N_i_t_samples_for_topic):
        N_i_t = N_i_t_samples_for_topic
        if N_i_t <= 0:
            return None
        mu_hat = alpha / (alpha + beta)
        try:
            log_term_numerator = (4 * K_total_kcs_in_unit * (N_i_t**2)) / delta
            if log_term_numerator <= 0:
                return None
            log_val = np.log(log_term_numerator)
            confidence_radius = np.sqrt(log_val / (2 * N_i_t))
        except Exception as e:
            return None
        LCB = mu_hat - confidence_radius
        UCB = mu_hat + confidence_radius
        if LCB >= masteryThreshold:
            self.A.discard(topicId)
            return 'remove'
        elif UCB < masteryThreshold:
            return 'weak'
        return None
...
def run_policy_simulation(policy_class, num_kcs: int, num_runs: int, config: AssessmentConfig):
    ...
    for _ in tqdm(range(num_runs), desc=f"Simulating {policy_class().name} ({num_kcs} KCs)"):
        ...
        if policy_class.__name__ == "HDoCPolicy":
            policy = HDoCPolicy(K_total_kcs_in_unit=K_total_kcs, delta=config.confidenceThreshold, mastery_threshold=config.masteryThreshold)
        else:
            policy = policy_class()
...
def main():
    ...
    for num_kcs in kc_counts:
        ...
        for policy_class in [ThompsonSamplingPolicy, RandomPolicy]:
        ...
        # Handle HDoCPolicy separately since it needs additional parameters
        print(f"Running HDoC...")
        policy_results = run_policy_simulation(
            lambda: HDoCPolicy(
                K_total_kcs_in_unit=num_kcs,
                delta=config.confidenceThreshold,
                mastery_threshold=config.masteryThreshold
            ),
            num_kcs=num_kcs,
        ...
        results['HDoC']['accuracy'].append(policy_results[0])
        results['HDoC']['avg_questions'].append(policy_results[1])
        results['HDoC']['std_questions'].append(0)
        print(f"HDoC Results:")
    ...
    plt.tight_layout()
    plt.savefig('simulation_results-3.png', dpi=200, bbox_inches='tight')
    plt.close()
